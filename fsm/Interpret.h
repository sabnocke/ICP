...
private:
  ...
  /// Časovač pro zpracování zpožděných přechodů
  Timing::Timer timer{};

  /**
   * @brief Provede změnu stavu na základě dostupných přechodů.
   * @param tg Skupina přechodů pro aktuální stav.
   */
  void ChangeState(const TransitionGroup &tg);

  /**
   * @brief Napojí časovače (delay) pro všechny relevantní přechody.
   */
  void LinkDelays();
